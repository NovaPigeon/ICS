...
#define SET_ROOT(i, ptr) (*(unsigned long *)(heap_classp + i * DSIZE) = (unsigned long)ptr)
...
int mm_init(void)
{
   ...
   mm_heap_lo = mem_heap_lo(); // 将空指针设置为mem_heap_lo，既可以使指针不在堆的可用区域内，又可以在压缩指针时避免溢出
 ...
 *  {1<<4},{1<<4+1 ~ 1<<5},{1<<5+1 ~ 1<<6},....,{1<<(CLASS_NUM+2)+1 ~ 1<<(CLASS_NUM+3)}
 *  使用线性查找获得其所在组别
 */
static int get_class_index(size_t size)
{
   ...
   {
      class_max_size = (1 << ((unsigned long)index + 4));
 ...
 * should point to A).
 * – All free list pointers points between mem heap lo() and mem heap high(). – Count free blocks by iterating through every block and traversing free list by pointers and see if
 * they match.
...
void mm_checkheap(int lineno)
{
   ...
   if (lineno & 0x2)
   {
      dbg_printf("Prilogue Header addr = 0x%lx , size = %u , curr_alloc = %u , prev_alloc = %u\n",
                 (unsigned long)hp, GET_SIZE(HDRP(hp)), GET_ALLOC_CURR(HDRP(hp)), GET_ALLOC_PREV(HDRP(hp)));
      dbg_printf("Prilogue Footer addr = 0x%lx , size = %u , curr_alloc = %u ,prev_alloc = %u\n",
                 (unsigned long)hp, GET_SIZE(FTRP(hp)), GET_ALLOC_CURR(FTRP(hp)), GET_ALLOC_PREV(FTRP(hp)));
      dbg_printf("Epilogue Header addr = 0x%lx , size = %u , curr_alloc = %u ,prev_alloc = %u\n\n",
                 (unsigned long)tp, GET_SIZE(HDRP(tp)), GET_ALLOC_CURR(HDRP(tp)), GET_ALLOC_PREV(HDRP(tp)));
   }
   /* 检查堆的总体情况 */
   if (lineno & 0x4)
   {
   ...
   if ((lineno & 0x8) || (lineno & 0x10) || (lineno & 0x20))
   {
      ...
      while (1)
      {
         ...
         if (lineno & 0x8)
         {
            ...
            {
               dbg_printf("ADDRESS UNALIGNED! index: %d;addr: 0x%lx;\n", index, (unsigned long)bp);
               error = error | 0x1;
            }
            ...
            if (!in_heap(bp))
            {
               dbg_printf("ADDRESS CROSS THE BORDER! index: %d;addr: 0x%lx;\n", index, (unsigned long)bp);
               error = error | 0x2;
            }
            ...
            {
               dbg_printf("TRUE LAST AND PREV ALLOC MISMATCH! index: %d;addr: 0x%lx;last_alloc: %d;prev_alloc:%d\n",
                          index, (unsigned long)bp, last_alloc, head_prev_alloc);
               error = error | 0x4;
            }
            if (head_size == 0)
               break;
            ...
            if (head_curr_alloc == 0)
            {
               ...
               {
                  dbg_printf("SIZE MISMATCH! index: %d;addr: 0x%lx;head_size: %d;foot_size:%d\n",
                             index, (unsigned long)bp, head_size, foot_size);
                  error = error | 0x8;
               }
               if (head_prev_alloc != foot_prev_alloc)
               {
                  dbg_printf("PREV ALLOC MISMATCH! index: %d;addr: 0x%lx;head_prev_alloc: %d;foot_prev_alloc:%d\n",
                             index, (unsigned long)bp, head_prev_alloc, foot_prev_alloc);
                  error = error | 0x8;
               }
               if (head_curr_alloc != foot_curr_alloc)
               {
                  dbg_printf("CURR ALLOC MISMATCH! index: %d;addr: 0x%lx;head_curr_alloc: %d;foot_curr_alloc:%d\n",
                             index, (unsigned long)bp, head_curr_alloc, foot_curr_alloc);
                  error = error | 0x8;
               }
            }
         }
         ...
         {
            nextp = NEXT_BLKP(bp);
            if ((head_curr_alloc == 0) && (GET_ALLOC_CURR(HDRP(nextp)) == 0))
            {
               dbg_printf("NOT COALESCED! Block (index: %d, addr: 0x%lx) and block (index: %d, addr: 0x%lx)",
                          index, (unsigned long)bp, index + 1, (unsigned long)nextp);
               error = error | 0x10;
            }
         }
         // 打印每个块的具体信息
         if (lineno & 0x20)
         {
            dbg_printf("index: %d;addr: 0x%lx; size: %d; curr_alloc: %d; prev_alloc:%d\n",
                       index, (unsigned long)bp, head_size, head_curr_alloc, head_prev_alloc);
         }
         if (error && error_exit)
         {
            dbg_printf("\n--------------\nBLOCK CHECK FINISH\n--------------\n");
   ...
   if (lineno & 0x80 || lineno & 0x40)
   {
      ...
      for (int i = 0; i < CLASS_NUM; ++i)
      {
         ...
         dbg_printf("CLASS %d:\n\tclass_addr: 0x%lx; root_block_addr: 0x%lx ;class_size: %d\n", i, (unsigned long)class_addr, (unsigned long)root_blk_addr, 1 << (i + 4));
         pred_ptr = class_addr;
         bp = root_blk_addr;
         cnt = 0;
         while (1)
         {
            ...
            if (lineno & 0x40)
            {
               ...
               if (!in_heap(bp))
               {
                  dbg_printf("\tPOINTER CROSS THE BORDER! id: %d; ptr: 0x%lx; mm_heap_lo: 0x%lx;mm_heap_hi: 0x%lx\n",
                             cnt, (unsigned long)bp, (unsigned long)mm_heap_lo, (unsigned long)mem_heap_hi);
                  error |= 0x20;
               }
               // 判断指针是否匹配
               if (GET_PRED(bp) != pred_ptr)
               {
                  dbg_printf("\tPOINTER MISMATCH! id: %d; ptr: 0x%lx; pred_ptr: 0x%lx\n",
                             cnt, (unsigned long)bp, (unsigned long)pred_ptr);
                  error |= 0x40;
               }
               ...
               if (i != class_index)
               {
                  dbg_printf("\tBLOCK SIZE MISMATCH CLASS SIZE! id: %d; blk_size: %d; class_size: %d; class_index: %d\n",
                             cnt, GET_SIZE(HDRP(bp)), 1 << (4 + i), class_index);
                  error |= 0x80;
               }
               if (error && error_exit)
                  exit(error);
            ...
            if (lineno & 0x80)
            {
               dbg_printf("\tINFO! id: %d; addr: 0x%lx; pred: 0x%lx; succ: 0x%lx; size: %d\n", cnt, (unsigned long)bp, (unsigned long)GET_PRED(bp), (unsigned long)GET_SUCC(bp), GET_SIZE(HDRP(bp)));
            }
            pred_ptr = bp;
            bp = GET_SUCC(bp);
         }
         dbg_printf("\tclass %d has %d blocks\n", i, cnt);
      }
