...
{
   /*
       显式空间链表的初始结构为，对齐块：4字节 0/1/1，序言块：16字节，16/1/1，pred 为 NULL，succ 为结尾块
       结尾块：8字节，0/1/1，pred 为序言块
   */
   ...
   SET_PRED(heap_tailp, heap_listp);     //结尾块pred为序言块
   SET_PRED(heap_listp, NULL);           //序言块pred为NULL
   SET_SUCC(heap_listp, heap_tailp);     //序言块succ为结尾块
   PUT(HDRP(heap_tailp), PACK(0, 1, 1)); //结尾块头部
   PUT(heap_tailp + WSIZE, PACK(0, 1, 1));
...
static void *extend_heap(size_t words)
{
   ...
   SET_SUCC(p, heap_tailp); // 结尾块为空闲块的 succ
   SET_PRED(heap_tailp, p);
...
static void *find_fit(size_t asize)
{
   //首次适配
   void *bp;
   for (bp = GET_SUCC(heap_listp); bp != heap_tailp; bp = GET_SUCC(bp))
   {
...
static void place(void *bp, size_t asize)
{
   ...
   if (csize - asize >= 4 * WSIZE)
   {
      ...
      SET_PRED(rest_ptr, pred_ptr);
      SET_SUCC(rest_ptr, succ_ptr);
      SET_PRED(succ_ptr, rest_ptr);
      SET_SUCC(pred_ptr, rest_ptr);
